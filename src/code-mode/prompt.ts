...
export function renderPromptSection(options: PromptSectionOptions): string {
  return [
    ...
    "",
    "For a single tool call with no computation, use tool-cli or a tool proxy instead — code mode",
    "earns its overhead when there is logic between the calls.",
    "",
    "### Finding tools",
    "",
    ...
    "### Results",
    "",
    "Most servers declare no output schema. When they do not, the result type is genuinely unknown —",
    "inspect it rather than assuming a shape:",
    "",
    "```javascript",
    'const result = await codemode.call("example-server", "example_list", { page: 1 });',
    ...
    "",
    "**Write ONE `code_execute` call that does the whole job.** Loops, comparisons, pagination, and",
    "aggregation all happen inside a single execution.",
    "",
    "```javascript",
    "const counts = {};",
    "let page = 1;",
