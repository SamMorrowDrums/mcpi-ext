...
export type { ServerInfo, ToolSummary, ToolDetails };

/**
 * JSON-RPC 2.0 server that exposes MCP tool discovery and execution.
 *
 * Methods:
...
export class ToolCliRpcServer {
  ...
  /** Returns the port the server is listening on (resolved after start for port 0). */
  getPort(): number {
    if (this.server) {
  ...
  private listServers(): { servers: ServerInfo[] } {
    const serverNames = this.mcpManager.getConnectedServers();
  ...
  private listTools(params: Record<string, unknown>): { server: string; tools: ToolSummary[] } {
    const server = requireString(params, "server");
    this.assertServerExists(server);
    const tools = this.mcpManager.getToolsForServer(server);
    return {
      server,
      tools: tools.map((t) => ({
        name: t.name,
  ...
  private describeTool(params: Record<string, unknown>): ToolDetails {
    ...
    const tool = this.mcpManager.getToolsForServer(server).find((t) => t.name === toolName);
    if (!tool) {
  ...
  private async callTool(params: Record<string, unknown>): Promise<{
    ...
    if (!client) {
      throw new RpcMethodError(-32603, `No client for server "${server}"`);
    }
  ...
  private assertServerExists(server: string): void {
    if (!this.mcpManager.getConnectedServers().includes(server)) {
      throw new RpcMethodError(
        -32602,
        `Server "${server}" not found. Connected: ${this.mcpManager.getConnectedServers().join(", ") || "(none)"}`,
      );
    }
  }
}
...
/** Pick up to `n` random tool names from a list. */
function pickRandomExamples(tools: McpTool[], n: number): string[] {
