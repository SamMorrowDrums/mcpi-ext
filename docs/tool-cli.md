...
The RPC server lives in the extension process, started on `session_start` and stopped on `session_shutdown`. The CLI binary uses `fetch` to call it. No new dependencies — Node's `http` module for the server, global `fetch` for the client.
...
Because it's shell-native, the agent gets bash superpowers for free:
